  ...
  (state.manholes||[]).forEach(function(mh){
    ...
    function openLabelEdit(){
      ...
      if(isRiser){
        ...
        var inpR=document.createElement('input');inpR.value=mh.label||'통신주입상';inpR.placeholder='시설명';inpR.style.cssText='font-size:15px;font-weight:600;color:#333;border:1px solid #bbb;border-radius:4px;background:#fff;outline:none;width:120px;text-align:center;padding:1px 3px';
        ...
        [inpR,selOwn,selSp].concat(selSurfR?[selSurfR,selPaveR]:[]).forEach(function(_el){_el.addEventListener('blur',onBlurR);_el.addEventListener('focus',onFocusR);});
        return;
      }
      // 현재 라벨 파싱: 앞부분 + M + (괄호안)
      var lab=mh.label||'';
      ...
      inpPre.addEventListener('blur',onBlur);inpIn.addEventListener('blur',onBlur);
      inpPre.addEventListener('focus',onFocus);inpIn.addEventListener('focus',onFocus);
      selKind.addEventListener('focus',onFocus);selKind.addEventListener('blur',onBlur);selSpec.addEventListener('focus',onFocus);selSpec.addEventListener('blur',onBlur);if(selSurf){[selSurf,selPave].forEach(function(_e){_e.addEventListener('focus',onFocus);_e.addEventListener('blur',onBlur);});}
    }
...
cv.addEventListener('pointerdown',function(e){
  ...
  else if(mode==='bpz1'){var wi=toWorld(e.clientX,e.clientY);var bp=nearBpPoint(wi[0],-wi[1]);bpFirst=bp?[bp.x,bp.y]:[wi[0],-wi[1]];mode='bpz2';setModeUI();toast('끝 보강판 측점을 클릭하세요');return;}
  else if(mode==='bpz2'){var wi=toWorld(e.clientX,e.clientY);var bp=nearBpPoint(wi[0],-wi[1]);var p2=bp?[bp.x,bp.y]:[wi[0],-wi[1]];if(!state.bpzones)state.bpzones=[];state.roadZones=[];pushHist();state.bpzones.push({p1:bpFirst,p2:p2,note:'보강판 지역',path:bpTracePath(bpFirst,p2)});bpFirst=null;bpHoverClear();bpPreviewClear();mode='pan';setModeUI();drawGeo();updMeta();toast('보강판 구역 생성');return;}
...
function importFromStage(id,srcStage){
  ...
  sb.from(srcTbl).select('*').eq('id',id).single().then(function(res){
    ...
    state.points=p.points||[];state.tangoEdit=p.tangoEdit||null;if(p.tangoManual)state.tangoManual=p.tangoManual;state.tgStore=p.tgStore||null;if(!state.tgStore&&(p.tangoEdit||p.tangoManual)){state.tgStore={tango:{edit:p.tangoEdit,manual:p.tangoManual||{},segDel:{}}};}_tgCtx='tango';state.lines=p.lines||[];state.baseTexts=p.baseTexts||[];state.markups=(p.markups||[]);state.labelOff=p.labelOff||{};state.manholes=p.manholes||[];state.bpzones=p.bpzones||[];state.roadZones=p.roadZones||[];state.depthCheck=p.depthCheck||[];if(typeof classifyRoad==='function')classifyRoad();state.depthGround=p.depthGround||null;state._depthAlign=null;state.titleBlock=p.titleBlock||null;state.crs=p.crs||'5186';state.photoDir=p.photoDir||{};state.routingDone=false;state.asbuilt=p.asbuilt||null;state.nightShift=p.nightShift||null;state.fieldDone=p.fieldDone||null;state.tamsa=!!p.tamsa;state.finalCsv=p.finalCsv?(Array.isArray(p.finalCsv)?p.finalCsv:[p.finalCsv]):[];state.bizInfo=p.bizInfo||null;
...
(function(){
  ...
  var m=document.getElementById('fldManhole');if(m)m.onclick=function(){toast('맨홀도 제작 — 다음 단계에서 연결합니다');};var _fi=document.getElementById('fldImport');if(_fi)_fi.onclick=function(){openImportList('survey');};var _fdd=document.getElementById('fldDel');if(_fdd)_fdd.onclick=fieldDelProject;
...
function tgInspRender(){if(!_tgInsp)return;var sg=_tgSegs[_tgInsp.seg];if(!sg)return;var key=tgManualKey(sg);tgInspClear();if(typeof LV!=='undefined'&&LV){var _ctrl=['mh','riser','depth','code'];var _want=tgInspLayers(_tgInsp.steps[_tgInsp.step],sg);var _chg=false;_ctrl.forEach(function(k){var on=_want.indexOf(k)>=0?1:0;if((LV[k]?1:0)!==on){LV[k]=on;_chg=true;}});if(_chg){try{localStorage.setItem('layerVis',JSON.stringify(LV));}catch(e){}if(typeof applyLayerVis==='function')applyLayerVis();if(typeof drawGeo==='function')drawGeo();}}
...
function loadPhotos(){photoMap={};afterMap={};if(!online||!state.projectId){if(photoPanelOpen)refreshPhotoPanel();return;}
  sb.from(DB+'_photos').select('point_no,url').eq('project_id',state.projectId).then(function(res){(res.data||[]).forEach(function(r){var pn=String(r.point_no);if(/_A$/.test(pn))afterMap[pn.replace(/_A$/,'')]=r.url;else photoMap[pn]=r.url;});if(photoPanelOpen)refreshPhotoPanel();});}
    ...
    compressImage(f,1280,0.7).then(function(blob){
      ...
      return sb.storage.from('photos').upload(path,blob,{upsert:true,contentType:'image/jpeg'}).then(function(up){
        ...
        return sb.from(DB+'_photos').delete().eq('project_id',state.projectId).eq('point_no',no).then(function(){
          return sb.from(DB+'_photos').insert({project_id:state.projectId,point_no:no,url:url}).then(function(ins){if(ins.error)throw ins.error;photoMap[no]=url;ok++;});
        });
      });
