...
function drawGeo(){
  ...
  state.points.forEach(function(p,i){if(p._hideMark&&!isRiserPt(p))return;if((typeof LV!=='undefined')&&LV.bp===0&&/보강판/.test((p.no||'')+'|'+(p.code||'')))return;var isBp=/보강판/.test((p.no||'')+'|'+(p.code||''));var bpHide=isBp&&(typeof bpPtHidden==='function')&&bpPtHidden(p);var s=S(p.x,p.y);if(p._hyun){var _hxh=0.1875,_hxc=({b:'#4fc3f7',d:'#1976d2',s:'#8d6e63',bd:'#e53935',db:'#e53935'})[(p._tcode||'').toLowerCase()]||'#1a7a5e';gPts.appendChild(el('line',{x1:s[0]-_hxh,y1:s[1]-_hxh,x2:s[0]+_hxh,y2:s[1]+_hxh,stroke:_hxc,'stroke-width':1.4,'vector-effect':'non-scaling-stroke','stroke-linecap':'round','pointer-events':'none'}));gPts.appendChild(el('line',{x1:s[0]-_hxh,y1:s[1]+_hxh,x2:s[0]+_hxh,y2:s[1]-_hxh,stroke:_hxc,'stroke-width':1.4,'vector-effect':'non-scaling-stroke','stroke-linecap':'round','pointer-events':'none'}));return;}
    ...
    if(typeof IS_REALTIME!=='undefined'&&IS_REALTIME){(function(pno){hit.addEventListener('pointerdown',function(ev){if(mode!=='pan')return;var self=this;if(self._lp)clearTimeout(self._lp);self._lp=setTimeout(function(){self._lp=null;self._lpFired=true;if(typeof rtPointMenu==='function')rtPointMenu(pno);},1000);});function _cl(){if(hit._lp){clearTimeout(hit._lp);hit._lp=null;}}hit.addEventListener('pointerup',_cl);hit.addEventListener('pointermove',_cl);hit.addEventListener('pointercancel',_cl);})(p.no);}
...
var _dwFn=null;
function _dwStart(e,fn){_dwLine=true;_dwLX0=e.clientX;_dwLY0=e.clientY;_dwLX=e.clientX;_dwLY=e.clientY;_dwPanning=false;_dwFn=fn||null;try{cv.setPointerCapture(e.pointerId);}catch(_dse){}}
...
cv.addEventListener('pointerdown',function(e){
  ...
  if(mode==='pan'){/* 측점/맨홀 위=선택(pointerup), 빈 곳 드래그=화면이동 */
  ...
  else if(mode==='bpz1'){var wi=toWorld(e.clientX,e.clientY);var bp=nearBpPoint(wi[0],-wi[1]);_dwStart(e,function(){bpFirst=bp?[bp.x,bp.y]:[wi[0],-wi[1]];mode='bpz2';setModeUI();toast('끝 보강판 측점을 클릭하세요');});return;}
  else if(mode==='bpz2'){var wi=toWorld(e.clientX,e.clientY);var bp=nearBpPoint(wi[0],-wi[1]);var p2=bp?[bp.x,bp.y]:[wi[0],-wi[1]];_dwStart(e,function(){if(!state.bpzones)state.bpzones=[];state.roadZones=[];pushHist();state.bpzones.push({p1:bpFirst,p2:p2,note:'보강판 지역',path:bpTracePath(bpFirst,p2)});bpFirst=null;bpHoverClear();bpPreviewClear();mode='pan';setModeUI();drawGeo();updMeta();toast('보강판 구역 생성');});return;}
    ...
    var _s=(ns.pt&&ns.d<snapTol)?[ns.pt[0],ns.pt[1]]:null;_dwStart(e,function(){if(_s){lineDraft.push(_s);renderDraft();}else if(typeof toast==='function')toast('측점 위를 클릭하세요 — 측점끼리만 연결됩니다');});return;}
...
function endPtr(e){
  if(_dwLine){var _wasPan=_dwPanning;_dwLine=false;_dwPanning=false;try{cv.releasePointerCapture(e.pointerId);}catch(_dre){}if(!_wasPan&&_dwFn){try{_dwFn();}catch(_dfe){}}_dwFn=null;return;}
