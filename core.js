...
cv.addEventListener('pointerdown',function(e){
  ...
  else if(mode==='bpz1'){var wi=toWorld(e.clientX,e.clientY);var bp=nearBpPoint(wi[0],-wi[1]);pendAct=function(){bpFirst=bp?[bp.x,bp.y]:[wi[0],-wi[1]];mode='bpz2';setModeUI();toast('끝 보강판 측점을 클릭하세요');};startPanFrom(e);return;}
  else if(mode==='bpz2'){var wi=toWorld(e.clientX,e.clientY);var bp=nearBpPoint(wi[0],-wi[1]);var p2=bp?[bp.x,bp.y]:[wi[0],-wi[1]];pendAct=function(){if(!state.bpzones)state.bpzones=[];state.roadZones=[];pushHist();state.bpzones.push({p1:bpFirst,p2:p2,note:'보강판 지역',path:bpTracePath(bpFirst,p2)});bpFirst=null;bpHoverClear();bpPreviewClear();mode='pan';setModeUI();drawGeo();updMeta();toast('보강판 구역 생성');};startPanFrom(e);return;}
